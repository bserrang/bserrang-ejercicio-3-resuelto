...
function getUsers() {
  fetch("./users.json")
    .then(resp => resp.json())
    .then(data => filterShowUsers(data));
}

function filterShowUsers(users) {
  const filteredUsers = users.filter(user => user.money > 500);
  filteredUsers.forEach(filteredUser => console.log(filteredUser.name));
}
